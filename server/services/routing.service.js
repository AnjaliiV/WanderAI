...
async function getRoutes(originQuery, destLat, destLng) {
  try {
    ...
    // Fallback to Haversine distance if OSRM fails (e.g. across oceans or timeouts)
    if (!distanceKm && originCoords) {
      distanceKm = calculateHaversineDistance(originCoords.lat, originCoords.lng, destLat, destLng) * 1.3; // 1.3 multiplier for road winding
      driveTimeH = Math.round((distanceKm / 60) * 10) / 10; // Assume 60km/h average
    }
